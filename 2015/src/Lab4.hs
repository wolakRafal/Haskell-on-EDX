...
euclid (x, y)
    | x == y = x
    | x < 0 || y < 0 = error "smth is wrong with this numbers"
    | otherwise = if x < y then euclid (x, (y - x)) else euclid ((x - y) ,y)
...
funkyMap f g xs = [if even pos then f x else g x | (pos,x) <- zip [0..] xs ]
